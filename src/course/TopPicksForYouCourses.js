...
class TopPicksForYouCourses extends Component {
    constructor(props) {
        super(props);
        this.state = {
            courses: [],
    ...
    render() {
        return (
            ...
            tabName = 'Top Picks in Learn Opus For You' handleLoadMore = {this.handleLoadMore} 
            isLoading = {this.state.isLoading} page = {this.state.page} noDataFoundMsg = 'No Top Picks Found Yet.'
            seeAllLink = {`/courseDetailList/${this.props.currentUser.laUserId}/toppicks`} tabIcon = {<ReadOutlined />} tabKey = {'1'}  isCourse = {true} isUser = {false} isCategory = {false}/>
        );
    }
}

export default withRouter(TopPicksForYouCourses);