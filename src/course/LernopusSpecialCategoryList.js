...
import { getAllSpecialCategories, getAllSpecialCategoriesForSearch} from '../util/APIUtils';
...
class LernopusSpecialCategoryList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            categories: [],
    ...
    loadCategoryList(page = 0, size = CATEGORY_LIST_SIZE) {
        ...
        {
            promise = getAllSpecialCategoriesForSearch(page, size, this.props.searchValue);
        }
        else
        {
    ...
    componentWillReceiveProps(nextProps){
        ...
        {
            promise = getAllSpecialCategoriesForSearch(0, 5, nextProps.searchValue);
        }
        else
        {
            promise = getAllSpecialCategories(0, 5);
        }
        if(!promise) {
            return;
        }
    ...
    render() {
        return (
            ...
            tabName = 'Learn Opus Special Categories' handleLoadMore = {this.handleLoadMore} 
            isLoading = {this.state.isLoading} page = {this.state.page} noDataFoundMsg = 'No Special Courses Found.'
            seeAllLink = {`/categoryDetails/special/true`} tabIcon = {<ReadOutlined />} tabKey = {'1'} isCourse = {false} isUser = {false} isCategory = {true}/>) : null)
                : (<TabWithScrollableCardComponent data = {this.state.categories} seeAll = 'See All' 
                tabName = 'Learn Opus Special Categories' handleLoadMore = {this.handleLoadMore} 
                isLoading = {this.state.isLoading} page = {this.state.page} noDataFoundMsg = 'No Special Courses Found.'
                seeAllLink = {`/categoryDetails/special/true`} tabIcon = {<ReadOutlined />} tabKey = {'1'} isCourse = {false} isUser = {false} isCategory = {true}/>)
        );
    }
}

export default withRouter(LernopusSpecialCategoryList);