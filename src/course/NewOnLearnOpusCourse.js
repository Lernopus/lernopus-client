...
class NewOnLearnOpusCourse extends Component {
    constructor(props) {
        super(props);
        this.state = {
            courses: [],
    ...
    render() {
        return (
            ...
            tabName = 'Latest Courses On Learn Opus' handleLoadMore = {this.handleLoadMore} 
            isLoading = {this.state.isLoading} page = {this.state.page} noDataFoundMsg = 'No Latest Courses Found.'
            seeAllLink = {`/courseDetailList/${this.props.currentUser.laUserId}/latest`} tabIcon = {<ReadOutlined />} tabKey = {'1'}  isCourse = {true} isUser = {false} isCategory = {false}/>
        );
    }
}

export default withRouter(NewOnLearnOpusCourse);