...
class TopTrendingCourses extends Component {
    constructor(props) {
        super(props);
        this.state = {
            courses: [],
    ...
    render() {
        return (
            ...
            tabName = 'Trending Courses in Learn Opus' handleLoadMore = {this.handleLoadMore} 
            isLoading = {this.state.isLoading} page = {this.state.page} noDataFoundMsg = 'No Trending Courses Found Yet.'
            seeAllLink = {`/courseDetailList/${this.props.currentUser.laUserId}/trending`} tabIcon = {<ReadOutlined />} tabKey = {'1'}  isCourse = {true} isUser = {false} isCategory = {false}/>
        );
    }
}

export default withRouter(TopTrendingCourses);