...
class CategoryDetailCourseList extends Component {
    ...
    loadCategoryCourseList(categoryId, page = 0, size = USER_LIST_SIZE) {
        let promise;
        promise = getCategoryCourseList(categoryId, page, size, this.props.currentUser.laUserId);
        if(!promise) {
            return;
        }
    ...
    componentDidMount() {
        ...
        this.loadCategoryCourseList(categoryId);
    }
    ...
    handleLoadMore() {
        if(!this.state.isLoading)
        {
            if(((this.state.page + 1) < this.state.totalPages))
            {
                ...
                {
                    const categoryId = this.props.match.params.categoryId;
                    this.loadCategoryCourseList(categoryId,this.state.page + 1);
                }
            }
        }
    }
